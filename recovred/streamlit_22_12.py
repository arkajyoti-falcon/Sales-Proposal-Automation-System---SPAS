...
- Each bullet MUST be short, clear, and client-friendly.
- **CRITICAL COUNTING RULES:**
  * **ONLY show numeric counts for CHUTES** (e.g., "127 gravity chutes", "59 non-gravity chutes")
  * For ALL other components, keep descriptions GENERIC without counts (e.g., "Auto Induct Line", "Infeed conveyor system", "Manual Induct Station")
  * DO NOT add counts for: Auto Induct, VDS, Feedlines, Conveyors, Operators, or any non-chute components
- **ALWAYS include throughput in the FIRST bullet** using the format: "Sorter based on [CBS type] technology, offers a designed throughput of [PPH] pph"
- If PPH is not provided, use: "Sorter based on [CBS type] technology"
- The order of bullets should vary slightly between generations.
- Bold the main components of the system. There can be max 2-3 bold words per bullet.
- **Examples of CORRECT bullet formatting:**
  * "**Sorter** based on cross-belt technology, offers a designed throughput of 11,000 pph"
  * "**Auto Induct Line** for precise, dimension-based parcel induction"
  * "**Infeed conveyor system** with telescopic units"
  * "**127 gravity chutes** and **59 non-gravity chutes** for efficient parcel collection"
  * "**Manual Induct Station** at ground level"
- **Examples of INCORRECT formatting (DO NOT USE):**
  * "Auto Induct Line – 8 fully automatic units" ❌ (no counts for non-chutes)
  * "Loop CBS – equipped with 11 VDS buffer units" ❌ (no counts for VDS)
  * "7 Nos of Feedlines" ✅ (exception: feedlines CAN have counts if it's a key differentiator)
...
def call_groq_exec_summary(system_text: str, client_name: str, project_title: str, pph_count: str = "", cbs_type: str = "") -> str:
    ...
    user_content = (
        ...
        "**CRITICAL:** Only show numeric counts for CHUTES. Keep all other components generic.\n"
        "**CRITICAL:** Include throughput (PPH) in the first bullet point."
    )
...
def build_proposed_system_description_section(doc, counter, client_name, project_name, 
    ...
    for line in flow_lines:
        ...
        if in_output_chutes and len(line) > 2 and line[0].isalpha() and line[1] == '.':
            ...
            # Split component title from description (separated by " - ")
            if " - " in clean_line:
                component_title = clean_line.split(" - ")[0].strip()
                description = " - " + " - ".join(clean_line.split(" - ")[1:])
            else:
    ...
    # 5.5 System Description (Detailed) - AI GENERATED WITH CONVEYOR BOQ TABLE
    if system_description_text:
        doc.add_paragraph("")
        add_numbered_subheading(doc, "System Description", f"{counter}.5")
        
        # **CHANGED: Insert text and replace [[CONVEYOR_BOQ_TABLE]] marker with actual table**
        lines = system_description_text.splitlines()
        table_inserted = False

        # Image replacements for specific sub-components
        base_dir = Path(__file__).parent
        placeholder_images = {
            "weighing conveyor": base_dir / "FIXED_IMAGE" / "weigh_conv.PNG",
            "buffer conveyor": base_dir / "FIXED_IMAGE" / "buffer_conv.PNG",
            "orientation conveyor": base_dir / "FIXED_IMAGE" / "oriant_conv.PNG",
            "merge conveyor": base_dir / "FIXED_IMAGE" / "merge_conv.PNG",
        }
        ...
        for i, line in enumerate(lines):
            ...
            if "image placeholder" in line_stripped.lower():
                comp_name = line_stripped
                comp_key = comp_name.lower()
                matched_path = None
                for key, img_path in placeholder_images.items():
                    if key in comp_key:
            ...
            # Regular text line - insert as paragraph
            p = doc.add_paragraph(line)
            apply_normal_style(p)
        ...
        try:
                ...
                with st.spinner("🔧 Continuing AI content generation..."):
                    try:
                        ...
                        if include_exec_summary and process_flow_text:
                            ...
                            exec_summary_text = call_groq_exec_summary(process_flow_text, client_name, project_name,pph_count=pph_count, cbs_type=cbs_type_detected )
            ...
            if include_proposed_system and process_flow_text:
                build_proposed_system_description_section(doc, counter, client_name, project_name, 
                                                         process_flow_text, layout_png_path, costing_file=costing_file)
                counter += 1
            ...
            if include_proposed_system and system_description_text:
                ...
                for line in (system_description_text or "").splitlines():
                    ...
                    if "[[CONVEYOR_BOQ_TABLE]]" in line.strip():
                        # Insert the Conveyor BOQ table
                        conveyor_boq = tables.get("Conveyor BOQ", [])
                        if conveyor_boq and len(conveyor_boq) > 1:
                            doc.add_paragraph("Conveyor BOQ:")
                            rows = len(conveyor_boq)
                            cols = len(conveyor_boq[0])
                            tbl = doc.add_table(rows=rows, cols=cols)
                            tbl.style = "Table Grid"
                            for r in range(rows):
                                for c in range(cols):
                                    tbl.cell(r, c).text = str(conveyor_boq[r][c])
                            doc.add_paragraph("")
                        continue
